...
export default function PostCard({ post, dbUserId }: { post: Post, dbUserId: string | null }) {
    ...
    const [hasLike, setHasLike] = useState(post.likes.some((like) => like.userId === dbUserId));
    ...
    return (
        <Card className="overflow-hidden">
            <CardContent className="p-4 smm:p-6">
                <div className="space-y-4">
                    <div className="flex space-x-3 sm:space-x-4">
                        <Link href={`/u/${post.author.username}`}>
                            <Avatar className="size-8 sm:size-10">
                                <AvatarImage src={post.author.image || "/avatar.png"} alt={post.author.name || ""} />
                            </Avatar>
                        </Link>
                    ...
                    <div className="flex items-center pt-2 space-x-2">
                        {user ? (
                            <Button
                                variant={"ghost"}
                                size={"sm"}
                                className={`text-muted-foreground gap-2 ${hasLike ? "text-red-500 hover:text-red-600" : "hover:text-red-500"
                                    }`}
                                onClick={handleLike}
                            >
                                {hasLike ? (
                                ...
                                {post.comments.map((comment) => (
                                    <div key={comment.id} className="flex space-x-3">
                                        <Avatar className="size-8 flex-shrink-0">
                                            <AvatarImage src={comment.author.image || "/avatar.png"} alt={comment.author.name || ""} />
                                        </Avatar>
